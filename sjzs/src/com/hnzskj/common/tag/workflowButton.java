...
public class workflowButton extends RequestContextAwareTag{
...
//    }
	...
	protected int doStartTagInternal() throws Exception {
		...
		sb.append("}\n");
		//获取人员列表
		//"<%=basePath %>notice!getOrgTree.action?time="+new Date().getTime();
		//system/empl!getEmpAll.action\
		...
		sb.append("<td>\n");
		if(tachelist.size()>1){
		sb.append("<select name=\"flow.nextinfoid\"  style=\"width:200px\" onchange=\"getNextTache(this.value)\" id =\"tache\">\n");
		...
		sb.append("<input type=\"hidden\" name=\"flow.nextinfoname\"  value=\"\" readonly=readonly/>\n");
		sb.append("</select>\n");
		}else{
			...
			sb.append("<input type=\"text\" name=\"flow.nextinfoname\" size=\"50\" style=border:none value="+tachelist.get(0).getTache_name()+" readonly=readonly\n/>");
		}
		sb.append("</td>\n");
		sb.append("</tr>\n");
		...
		if (null != nextTache) {
			...
			if (nameModel == 2) {
				...
				sb.append("<td>\n");
				sb
						.append("<input type=\"text\" name=\"empName\" size=\"30\"/><input type=\"hidden\" name=\"flow.nextEnti_ids\" /> <input type=\"button\" value=\"请选择人员\" onclick=\"getPerson()\">");
				sb.append("</td>\n");
				sb.append("</tr>\n");
			}
