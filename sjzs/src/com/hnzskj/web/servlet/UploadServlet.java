...
public class UploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		while (it.hasNext()) {
			...
			if ("journalID".equals(item.getFieldName())) {
				journalID = item.getString();
			}
			if (!item.isFormField()) {
		...
		String info = save(fileName, fileType, file, journalID, length1);
	...
	private String save(String fileName, String fileType, InputStream file,
			String journalID, long fileLength) {
		...
		try {
			/*
			 * sql.append("update journal set attachName = ? , attachType = ? , attachContent = ? where journalID = ? "
			 * ); pstmt = conn.prepareStatement(sql.toString());
			 * pstmt.setString(1, fileName); pstmt.setString(2, fileType);
			 * pstmt.setBinaryStream(3, file, (int) fileLength);
			 * pstmt.setString(4, journalID);
			 */
