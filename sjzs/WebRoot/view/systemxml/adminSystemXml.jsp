...
<html>
	...
	-->
		...
		function addConfirm(selectName) {
			...
			if((name != "") && (name != null)) {
				name=encodeURI(name);
				location.href = "systemXmlAction!add.action?nodeName=" + nodeName + "&name=" + name;
			} else {
				return false;
			}
		...
		function delConfirm(selectName) {
			...
			if(name == null) {
				...
				location.href = "systemXmlAction!del.action?nodeName=" + nodeName + "&code=" + code;
			} else {
				return false;
			}
		}
		...
		function updateConfirm(selectName) {
			...
			if((oldName != null) && (oldName != "")) {
				...
				if((newName != null) && (newName != "")) {
					...
					window.location.href="systemXmlAction!update.action?nodeName=" + nodeName + "&code=" + code + "&newName=" + newName;
				} else {
					return false;
				}
			} else {
		...
		function updateAll(selectName,reCode) {
			...
			if((oldName != null) && (oldName != "")) {
				if((newName != null) && (newName != "")){
					newName = encodeURI(newName);
				window.location.href="systemXmlAction!update.action?nodeName=" + selectName +"&code =" + reCode+"&newName="+newName ;
				}else{
					return false;
				}
			} else {
		...
		function addCodeTemp(){
			...
			if(null!=returnValue&&undefined!=returnValue){
				if(returnValue){
					this.location.href="systemXmlAction!adminlistSystemXml.action";
				}
			}
		}
		//删除模板项目编码配置
		function deleteCodeTemp(selectNode,id){
			...
			if(window.confirm("您确定要删除:" + name)) {
				...
				this.location.href = "systemXmlAction!delByCodeTemp.action?nodeName=" + selectNode + "&code=" + grade;
			}
		}
		...
		function updateCodeTemp(id){
			if(confirm("您确定要保存修改吗?")){
				...
				this.location.href = "systemXmlAction!updateByCodeTemp.action?nodeName=codeTemp" + "&code=" + grade+"&codeTemp.name="+name+"&codeTemp.chose="+chose+"&codeTemp.code="+code;	
			}
		}
			...
			<td align="right">
				...
				<td>
					<s:if test="#request.kuckfw.code==1">
						个人<input onclick="updateKu()" type="radio" checked="checked" name="fanwei" value="1" />
					</s:if>
					<s:else>
						个人<input  onclick="updateKu()"type="radio"  name="fanwei" value="1" />
					</s:else>
					
					<s:if test="#request.kuckfw.code==2">
						所在部门<input  onclick="updateKu()" type="radio" checked="checked" name="fanwei" value="2" />
					</s:if>
					<s:else>
						所在部门<input  onclick="updateKu()" type="radio"  name="fanwei" value="2" />
					</s:else>
					
					<s:if test="#request.kuckfw.code==3">
						所在子公司<input  onclick="updateKu()" type="radio" checked="checked" name="fanwei" value="3" />
					</s:if>
					<s:else>
						所在子公司<input  onclick="updateKu()" type="radio"  name="fanwei" value="3" />
					</s:else>
					
					<s:if test="#request.kuckfw.code==4">
						所有<input  onclick="updateKu()" type="radio" checked="checked" name="fanwei" value="4" />
					</s:if>
					<s:else>
						所有<input onclick="updateKu()" type="radio"  name="fanwei" value="4" />
					</s:else>
				</td>
				</tr>
				<tr>
