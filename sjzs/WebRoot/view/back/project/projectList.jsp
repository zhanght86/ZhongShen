...
<head>
	...
	<script type="text/javascript">
		...
		function doUpdate(proId) {
			var selectVal = getCheckBox(proId,1);
			if ("" != selectVal) {
				...
				var goURI = "system/project!goUpdatePage.action?project.proId="+selectVal;
				self.parent.addTab('产品信息',goURI,'icon-add')
			} else {
				alert("有且只有一个复选框必须被选中");
			}
		}
		...
		function doDelete(proId){
			...
			if ("" != selectVal) {
				...
				if (confirm("您确定要删除所选的信息？")) {
					var url = "system/project!delProject.action?project.proId="+selectVal;
...
<body>
<form action="system/project!searchProject.action" method="post" name="queryForm" style="display: none;">
	<input type="hidden" value="<s:property value="page.curPage"/>" name="page.curPage" id="reqPage"/>
	<input type="hidden" name="project.proNo" value="${project.proNo }"/>
	<input type="hidden" name="project.proName" value="${project.proName }"/>
</form>
<div align="center" style="margin-top:10px;">
  <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      ...
      <form action="system/project!searchProject.action" method="post" id="searchF" name="searchF">
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <label>产品编号:</label><input type="text" name="project.proNo" value="${project.proNo }"/>
			<label>产品名称:</label><input type="text" name="project.proName" value="${project.proName }"/>
            <input type="image" src="images/ny_searchbtn.jpg"/>
          </td>
          <td width="49%" valign="middle" align="right">&nbsp;
          	<c:if test="${fu:check('system/project!goAddPage.action')}">
          		<img src="images/ny_tj.jpg" width="19" height="19" align="absmiddle" />
				<a href="javascript:void(0)" title="新增产品" onclick="self.parent.addTab('新增产品','system/project!goAddPage.action','icon-add')" class="txt12">新增产品</a>
			</c:if>
          	&nbsp; &nbsp;
          	<c:if test="${fu:check('system/project!goUpdatePage.action')}">
          		<img src="images/edit.png" width="19" height="19" align="absmiddle" />
          		<a href="javascript:void(0)" title="编辑产品" onclick="javascript:doUpdate('proId');" class="txt12">编辑</a>
          	</c:if>          
          	&nbsp; &nbsp;
          	<c:if test="${fu:check('system/project!delProject.action')}">
          		<img src="images/ny_sc.jpg" width="19" height="19" align="absmiddle" /> 
          		<a href="javascript:void(0)" title="删除产品" onclick="javascript:doDelete('proId')" class="txt12">删除</a>
          	</c:if>
          	</td>
        </tr>
      </table>
				...
				<s:else>
					<tr>
		  			...
		  			<td align="center" align="center">
		  				<input type="checkbox"  id="proId" name="proId" value="${entry.proId }"/>
		  			</td>
		  			<td class="txt03467B">
		  				<a href="javascript:void(0);" onclick="self.parent.addTab('产品信息','system/project!goUpdatePage.action?project.proId=${entry.proId}','icon-add')">${entry.proNo }</a>
		  			</td>
		  			<td>		  			
		  				<a href="javascript:void(0);" onclick="self.parent.addTab('产品信息','system/project!goUpdatePage.action?project.proId=${entry.proId}','icon-add')">${entry.proName }</a>					  			
		  			</td>
		  			<td align="center">
			  			<a href="javascript:void(0);" onclick="self.parent.addTab('产品信息','system/project!goUpdatePage.action?project.proId=${entry.proId}','icon-add')">${entry.proOrder }</a>
		  			</td>
		  			<td align="center">
		  				<a href="javascript:void(0);" onclick="self.parent.addTab('产品信息','system/project!goUpdatePage.action?project.proId=${entry.proId}','icon-add')">${fn:substring(entry.proDateTime,0,19) }</a>
		  			</td>
		  		</tr>
		  	</s:iterator>
		  	<tr class="row_bg">
