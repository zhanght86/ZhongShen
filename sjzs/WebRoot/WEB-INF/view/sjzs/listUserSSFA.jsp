...
<html>
  <head>
	...
	function deleteSSFA(jmcode) {
			var checkboxs = document.getElementsByName("shiShiFangAnId"); //(jmcode,2);
			var values = '';
			...
			if ("" != values) {
			if(confirm("您确定要删除么？")) {
				//var goURI = "system/ssfa!deleteSSFAs.action?ssfn.id=" + values+"&shiShiFangAn.sort=${shiShiFangAn.sort}";;
		...
		function downloadAttach(curchaId) {
		 ...
		 $(document).ready(function() {
			  ...
			  $("input[name='shiShiFangAnId']").each(function(){
		 ...
		 $(function(){
				...
				dymShowText(4,10,150);
				dymShowText(5,10,150);
			});
			
			function search(){
				$('#reqPage').val(0);
				var newName = $('#textfield').val();
				var oldName = "${ssfn.caption}";
				if(newName == oldName){
					$('#textfield').val("");
				}
			}
		    ...
		    function moveDataBack(){
		    	var checkboxs = document.getElementsByName("shiShiFangAnId"); 
				var values = '';
				for(var i = 0; i<checkboxs.length ;i++){
...
<div id="navTab" class="tabsPage">
 ...
 <form id="sysUpload" name="sysUpload" action="system/ssfa!searchByCondition.action" method="post">
	<input type="hidden" value="${param.menuType}" name="menuType" id="menuType" />
	<input type="hidden" name="shiShiFangAn.name" id="caption" value="${ssfn.caption }"/>
	<input type="hidden" name="shiShiFangAn.sort" value="${ssfn.parentId}" />
 </form>
 <form id="deleteForm" name="deleteForm" action = "" method="post" style="display: none;">
  ...
  <form rel="pagerForm" action="system/clientUpload!list.action" method="post" id="searchF" name = "searchF" onsubmit="return showSys();">
   <input type="hidden" value="<s:property value="pageSSFN.curPage"/>" name="pageSSFN.curPage" id="reqPage"/>
   <input type="hidden" value="${param.menuType}" name="menuType" id="menuType" />
	<div class="searchBar">
		<ul class="searchContent">
			<li>
				<label>案例名称：</label>
				 <input type="text" name="ssfn.caption" id="textfield" value="${ssfn.caption }"/>
...
<div class="pageContent">
	...
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc" layoutH="118">
		...
		<tbody id = 'playList'>
			<s:iterator id="entry" value="pageSSFN.list" status="sta">
				<tr rel="${sta.count }">
				<c:if test="${fu:check('system/clientUpload!moveData.action')}">
		  			<td align="center">
		  			<input type="checkbox" name="shiShiFangAnId" value="${entry.id }" transport="${entry.isTransport }" />
		  			</td>
		  			</c:if>
			  			<td>${sta.count+(pageSSFN.curPage-1)*pageSSFN.maxResult}</td>
			  			
			  			<td>
						<a href="javascript:void(0)" onclick="self.parent.parent.addTab('查看方案','system/clientUpload!showAtttachInfo.action?attachId =${entry.attachId }&type=2','icon-add')" >${entry.caption}</a>
						...
						<td>
							<a style="text-decoration:none;">${entry.keyWord}</a>
							<input type="hidden" value="${entry.keyWord}" />
						</td>
						<td>
							<fmt:parseDate value="${entry.writeDate}" var="date"/>
					...
					<div class="pages" style="text-align: center;width:95%">
						<table style="width: 100%">
						 <tr>
						  ...
						  <a style="color: #1E5494;">总记录数：${pageSSFN.totalRecords} 条</a>&nbsp;&nbsp;
	<c:choose>
		<c:when test="${ 1 == pageSSFN.curPage}">
			<a>首页</a>
			<a>上一页</a>
		</c:when>
		<c:otherwise>
			<a href="" onClick="goNextPage(1);return false;">首页</a>
			<a href="" onClick="goNextPage(${pageSSFN.curPage - 1 });return false;">上一页</a>
		</c:otherwise>
	</c:choose>
	<c:forEach begin="${pageSSFN.beginIndex}" end="${pageSSFN.endIndex}"
		var="pageCount" step="1">
		<c:choose>
			<c:when test="${ pageCount == pageSSFN.curPage}">
	...
	<c:choose>
		<c:when test="${ pageSSFN.curPage >= pageSSFN.totalPage}">
			<a>下一页</a>
			<a>末页</a>
		</c:when>
		<c:otherwise>
			<a href="" onClick="goNextPage(${pageSSFN.curPage + 1});return false;">下一页</a>
			<a href="" onClick="goNextPage(${pageSSFN.totalPage });return false;">末页</a>
		</c:otherwise>
	</c:choose>
						  </td>
						 </tr>
