...
<html>
  <head>
	...
	function deleteSSFA(jmcode) {
			...
			if ("" != values) {
			if(confirm("您确定要删除么？")) {
				var goURI = "system/ssfa!deleteSSFAs.action?shiShiFangAn.id=" + values+"&shiShiFangAn.sort=${shiShiFangAn.sort}";;
		...
		function addSSFA() {
			var reqURL = "<%=basePath%>system/ssfa!goToAddPage.action?shiShiFangAn.sort=${shiShiFangAn.sort}&nocache=" + new Date().getTime();
			self.parent.addFFAL(reqURL,'700','350')	
		}
...
<div id="navTab" class="tabsPage">
 ...
 <form id="userUpload" name="userUpload" action="system/clientUpload!list.action" method="post">
	...
	<input type="hidden" name="ssfn.caption" id="caption" value="${shiShiFangAn.name }"/>
	<input type="hidden" name="ssfn.parentId" value="${shiShiFangAn.sort}" />
 </form>
 <form id="deleteForm" name="deleteForm" action = "" method="post" style="display: none;">
  ...
  <form rel="pagerForm" action="system/ssfa!searchByCondition.action" method="post" id="searchF" name = "searchF" onsubmit="return showUserUpload();">
   ...
   <input type="hidden" value="${param.menuType}" name="menuType" id="menuType" />
	<div class="searchBar">
		<ul class="searchContent">
			<li>
				<label>案例名称：</label>
				 <input type="text" name="shiShiFangAn.name" id="textfield" value="${shiShiFangAn.name }"/>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li>
				<c:if test="${fu:check('system/ssfa!goToAddPage.action')}">
				 <a class="add" href="javascript:void(0)" onclick="addSSFA();" ><span>添加</span></a>
				</c:if>
			</li>
			<li>
				<c:if test="${fu:check('system/ssfa!deleteSSFAs.action')}">
					<a class="delete" href="javascript:deleteSSFA()"  title="确定要删除吗？" warn="请选择"><span>删除</span></a>
				</c:if>
			</li>
			<li>
				<c:if test="${fu:check('system/ssfa!goToUpdatePage.action')}">
					<a class="edit" href="javascript:void(0)" onclick="update('shiShiFangAnId')" warn="请选择"><span>修改</span></a>
				</c:if>
			</li>
	...
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc" layoutH="118">
		<thead>
			<tr><c:if test="${fu:check('system/ssfa!goToAddPage.action')}">	
		...
		<tbody id = 'playList'>
			<s:iterator id="entry" value="page.list" status="sta">
				...
				<c:if test="${fu:check('system/ssfa!goToAddPage.action')}">
		  			<td align="center">
		  			<input type="checkbox" name="shiShiFangAnId" value="${entry.id }"/>
		  			</td>
		  			</c:if>
			  			<td>${sta.count+(page.curPage-1)*page.maxResult}</td>
			  			
			  			<td>
						<a href="javascript:void(0)" onclick="self.parent.parent.addTab('查看方案','attach!showAtttachInfo.action?attach.attachId =${entry.attachId }&type=2','icon-add')" >${entry.name}</a>
	...
	<div class="panelBar">
					<div class="pages" style="text-align: center;width:95%">
						<table style="width: 100%">
						 <tr>
						  <td>	<span><font color="red">点击"方案名称"可查看详细信息！</font> </span></td>
